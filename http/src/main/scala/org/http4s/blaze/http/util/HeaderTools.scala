...
private[blaze] object HeaderTools {
  ...
  case class SpecialHeaders(transferEncoding: Option[String], contentLength: Option[String], connection: Option[String])
  ...
  def renderHeaders[H: HeaderLike](sb: StringBuilder, headers: Iterable[H]): SpecialHeaders = {
    val hl = implicitly[HeaderLike[H]]
    ...
    SpecialHeaders(transferEncoding, contentLength, connection)
  }
}
