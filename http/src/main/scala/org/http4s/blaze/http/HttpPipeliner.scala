...
final class HttpPipeliner(maxConcurrentRequests: Int, codec: HttpCodec, service: HttpService, ec: ExecutionContext) {
  ...
  private[this] def readStalled(): Boolean = requestQueue.size() >= maxConcurrentRequests
    ...
    if (requestQueue.peekFirst() eq request) {
      ...
      codec.renderResponse(response).onSuccess {
        case HttpCodec.Reload => lock.synchronized {
          ...
          if (readStalled()) ec.prepare().execute(new Runnable {
